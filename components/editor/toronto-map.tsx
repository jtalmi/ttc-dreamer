...
function pickSelectableFeature(features: MapGeoJSONFeature[] | undefined) {
  ...
  for (const feature of features) {
    ...
    const priority = SELECT_LAYER_PRIORITY.indexOf(
      layerId as (typeof SELECT_LAYER_PRIORITY)[number],
    );
...
type TorontoMapProps = Readonly<{
  ...
  pendingInterchangeSuggestion?:
    | (InterchangeSuggestion & { stationName: string })
    | null;
  /**
...
export default function TorontoMap({
  ...
  const [pendingStationName, setPendingStationName] =
    useState<PendingStationName | null>(null);
  ...
  const [draggingStationId, setDraggingStationId] = useState<string | null>(
    null,
  );
  const draggingRef = useRef<string | null>(null);
  // Suppress click after a station drag (set in mouseup, cleared in click)
  ...
  const activeLineColor = useMemo(() => {
    ...
    return (
      draft.lines.find((l) => l.id === drawingSession.lineId)?.color ??
      "#7B61FF"
    );
  }, [drawingSession, draft]);

  const inProgressGeoJSON = useMemo(() => {
    return buildInProgressGeoJSON(
      drawingSession,
      draft ?? {
        id: "",
        title: "",
  ...
  const buildBaselineStationInspection = useCallback(
    (
      ...
      layerId: string,
    ): BaselineStationInspection | null => {
      ...
      if (layerId === "ttc-stations-circle") {
        return {
          ...
          municipality:
            asString(props["CITY"]) ?? asString(props["MUNICIPALITY"]),
  ...
  const handleMouseMove = useCallback(
    (e: MapLayerMouseEvent) => {
      ...
      if (activeTool === "draw-line") {
        ...
        if (map && draft) {
          ...
          dispatch?.({
            ...
            payload: snapTarget?.position ?? null,
          });
        }
        return;
      }
      ...
      if (activeTool === "select" && draggingRef.current) {
        ...
        dispatch?.({
          type: "moveStation",
          payload: { stationId: draggingRef.current, position: lngLat },
        });
        return;
      }
      ...
      const stationFeature = (e.features ?? []).find(
      ...
      if (stationFeature) {
        ...
        const stationType: "ttc" | "go" =
          layerId === "go-stations-circle" ? "go" : "ttc";
    ...
    if (draggingRef.current) {
      ...
      if (lastDragPosition.current) {
        dispatch?.({
          type: "moveStation",
          payload: {
            stationId: draggingRef.current,
            position: lastDragPosition.current,
          },
        });
        lastDragPosition.current = null;
      }
   ...
   */
  const fireGeocodeUpdate = useCallback(
    (stationId: string, position: [number, number]) => {
  ...
  const handleClick = useCallback(
    (e: MapLayerMouseEvent) => {
      // Suppress click that follows a station drag gesture
      ...
      if (activeTool === "draw-line") {
              ...
              const waypoints =
                line.stationIds.length >= 2
                  ? (line.stationIds
                      .map(
                        (id) =>
                          draft.stations.find((s) => s.id === id)?.position,
                      )
        ...
        if (map && draft && draft.lines.length > 0) {
          for (const line of draft.lines) {
            ...
            const firstStation = draft.stations.find(
              (s) => s.id === line.stationIds[0],
            );
            const lastStation = draft.stations.find(
              (s) => s.id === line.stationIds[line.stationIds.length - 1],
            );
            ...
            if (distLast <= 20) {
              ...
              dispatch?.({
                ...
                payload: {
                  lineId: line.id,
                  mode: "extend",
                  initialStationId: lastStation.id,
                },
              });
              return;
            }
            if (distFirst <= 20) {
              ...
              dispatch?.({
                ...
                payload: {
                  lineId: line.id,
                  mode: "extend",
                  initialStationId: firstStation.id,
                },
              });
              return;
            }
        ...
        if (draft && dispatch) {
          ...
          const nextColor =
            DEFAULT_LINE_COLORS[
              draft.lines.length % DEFAULT_LINE_COLORS.length
            ];
          ...
          dispatch({
            type: "startDrawing",
            payload: { lineId: newLineId, mode: "new" },
          });
        ...
        for (const line of draft.lines) {
          ...
          const waypoints =
            line.stationIds.length >= 2
              ? (line.stationIds
                  .map(
                    (id) => draft.stations.find((s) => s.id === id)?.position,
                  )
                  .filter(Boolean) as [number, number][])
              : line.waypoints;
          if (waypoints.length < 2) continue;
        ...
        if (feature) {
          ...
          if (layerId === "proposal-stations-circle") {
            ...
            if (id) {
              ...
              dispatch?.({
                type: "inspectElement",
                payload: { id, elementType: "station" },
              });
            }
            return;
          }
          ...
          if (
            layerId === "ttc-stations-circle" ||
            layerId === "go-stations-circle"
          ) {
          ...
          if (
            layerId?.startsWith("ttc-line-") ||
            layerId === "go-routes-line"
          ) {
  ...
  const handleStationNameSave = useCallback(
    (name: string) => {
  ...
  const cursorStyle = useMemo(() => {
    ...
    switch (activeTool) {
      case "draw-line":
        return "crosshair";
      case "add-station":
        return isOverSegment ? "cell" : "not-allowed";
      case "select":
        return isOverSelectable ? "pointer" : "default";
      default:
        return "default";
    }
  ...
  return (
    ...
    >
      ...
      */}
      ...
      <TtcLayers
        routes={
          baselineMode === "future_committed"
            ? data.futureTtcRoutes
            : data.ttcRoutes
        }
        stations={
          baselineMode === "future_committed"
            ? data.futureTtcStations
            : data.ttcStations
        }
      />
      <ProposalLayers
      ...
      {hoverStation && (
        <Popup
          ...
          style={{ fontSize: "14px" }}
        >
          {hoverStation.name} &mdash;{" "}
          {hoverStation.type === "go" ? "GO" : "TTC"}
        </Popup>
      )}
