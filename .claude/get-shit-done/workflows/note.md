...
- **Global scope**: `/Users/jonathan/dev/ttc-dreamer/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/jonathan/dev/ttc-dreamer/.claude/notes/`)
...
2. Glob `/Users/jonathan/dev/ttc-dreamer/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/jonathan/dev/ttc-dreamer/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/jonathan/dev/ttc-dreamer/.claude/notes/` — works in any directory
